...
namespace SurfaceFields {
	void StripePatternsGlobalIntegration::globallyIntegrateOneComponent(const Surface& surf, const Eigen::MatrixXd& v, Eigen::VectorXd& scales, Eigen::VectorXd& theta) {
		...
		Eigen::VectorXd edge_one_forms = GetEdgeOneForms(surf, v, scales, face_areas);
	...
	Eigen::VectorXd StripePatternsGlobalIntegration::GetEdgeOneForms(const Surface& surf, const Eigen::MatrixXd& v, const Eigen::VectorXd& scales, const Eigen::VectorXd& face_areas) {
		size_t nedges = surf.data().E.rows();
		Eigen::VectorXd omega = Eigen::VectorXd::Zero(nedges);
		for (size_t eid = 0; eid < nedges; eid++) {
			Eigen::Vector3d e = (surf.data().V.row(surf.data().edgeVerts(1)) - surf.data().V.row(surf.data().edgeVerts(0))).transpose();
			double area_sum = 0;
			for (size_t id = 0; id < 2; id++) {
				...
				area_sum += face_areas[fid];
				Eigen::Vector3d Euclidean_vec = surf.data().Bs[fid] * (v.row(fid).transpose()); // The original vec   
				Euclidean_vec *= scales[fid] / Euclidean_vec[fid];	// rescaling that vector, where the scales are from the local solver for a better construction
				omega[eid] += Euclidean_vec.dot(e) * face_areas[fid];
			}
			omega[eid] /= area_sum;
		}
		return omega;
